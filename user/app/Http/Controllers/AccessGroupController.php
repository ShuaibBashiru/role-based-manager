...
class AccessGroupController extends Controller
{
    ...
    public function manage($id){
        ...
        if ($record['status']=='success') {
            $record = $record['info'];
            return view('apps.access_group.manage', compact('record'));
        }else{
            $message = json_encode("No record found");
            return redirect()->route('list_accessgroup')->with('message', json_encode($message));
        }
    }

    public function list(){
        return view('apps.access_group.record');
    }

    public function uploadBatch(){
        return view('apps.access_group.upload_record');
    }

    public function createRecord(){
        return view('apps.access_group.new_record');
    }

    public function checkIfIdIsValid($data){
        $query = AccessGroup::where('deleted_status', '=', '0')
    ...
    public function record($id)
    {
        ...
        $returnData = '';
        $result = [];
        $query = AccessGroup::from('access_group as t1')
                    ->where('t1.deleted_status', '=', '0')
                    ->where('t1.generated_id', $id)
                    ->leftJoin('statuses as t2', 't2.id', '=', 't1.status_id')
                    ->get(['t1.group_name', 't1.generated_id', 't1.updated_at'])->first();
    ...
    public function listAll()
    {
        ...
        $query = AccessGroup::from('access_group as t1')
                    ...
                    ->leftJoin('statuses as t2', 't2.id', '=', 't1.status_id')
                    ->orderBy('t1.id', 'DESC')
                    ->get(['t1.group_name', 't1.descriptions', 't1.date_created',
    ...
    public function create(Request $request){
       $request->validate([
            'group_name' => 'required|unique:access_group|max:100|min:2',
        ]);
        try {
            $exist = false;
            $failed = false;
            ...
            $record = [
                ...
                $query = AccessGroup::insert($record);
                if ($query) {
                $successful = true;
                }else{
                $failed = true;
...
try {
    ...
    $update = AccessGroup::where('generated_id', '=', $newid)
...
// End loop
    ...
    try {
        ...
        $update = AccessGroup::where('generated_id', '=', $id)
                            ->update($row);
...
$query = AccessGroup::where('generated_id', '=', $id)
