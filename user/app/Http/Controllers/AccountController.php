...
use App\Http\Requests\AccountRequestBatch;
...
class AccountController extends Controller
{
    ...
    public function manage(Request $request, $id){
        $securedata = $request->session()->get('securedata');
        if ($id === $securedata['generated_id']) {
        ...
        if ($record['status']=='success') {
            $record = $record['info'];
            return view('apps.account.manage', compact('record'));
        }else{
            $message = [
                "type" => "error",
    ...
    public function profile(Request $request){
        $securedata = $request->session()->get('securedata');
        $id = base64_decode(base64_decode($securedata['generated_id']));
        $record = $this->record($id);
        if ($record['status']=='success') {
            $record = $record['info'];
            return view('apps.account.profile', compact('record'));
        }else{
            $message = [
                "type" => "error",
                "info" => "Something went wrong! refresh and continue.",
    ...
    public function edit_name(Request $request, $id){
        ...
        if ($id === $securedata['generated_id']) {
            ...
            if ($record['status']=='success') {
                $record = $record['info'];
                return view('apps.account.edit_name', compact('record'));
            }else{
                $message = [
                "type" => "info",
                "info" => "",
    ...
    public function edit_dob(Request $request, $id){
        $securedata = $request->session()->get('securedata');
        if ($id === $securedata['generated_id']) {
            $id = base64_decode(base64_decode($id));
            $record = $this->record($id);
            if ($record['status']=='success') {
                $record = $record['info'];
                return view('apps.account.edit_dob', compact('record'));
            }else{
                $message = [
                "type" => "info",
                "info" => "",
    ...
    public function edit_gender(Request $request, $id){
        $securedata = $request->session()->get('securedata');
        if ($id === $securedata['generated_id']) {
            $id = base64_decode(base64_decode($id));
            $record = $this->record($id);
            if ($record['status']=='success') {
                $record = $record['info'];
                return view('apps.account.edit_gender', compact('record'));
            }else{
                $message = [
                "type" => "info",
                "info" => "",
    ...
    public function edit_contact_address(Request $request, $id){
        $securedata = $request->session()->get('securedata');
        if ($id === $securedata['generated_id']) {
            $id = base64_decode(base64_decode($id));
            $record = $this->record($id);
            if ($record['status']=='success') {
                $record = $record['info'];
                return view('apps.account.edit_contact_address', compact('record'));
            }else{
                $message = [
                "type" => "info",
                "info" => "",
    ...
    public function edit_phone(Request $request, $id){
        ...
        if ($id === $securedata['generated_id']) {
            ...
            if ($record['status']=='success') {
                $record = $record['info'];
                return view('apps.account.edit_phone', compact('record'));
            }else{
                $message = [
                "type" => "info",
                "info" => "",
    ...
    public function edit_password(Request $request, $id){
        ...
        if ($id === $securedata['generated_id']) {
            ...
            if ($record['status']=='success') {
                $record = $record['info'];
                return view('apps.account.edit_password', compact('record'));
            }else{
                $message = [
                "type" => "info",
                "info" => "",
    ...
    public function list(){
        return view('apps.account.record');
    }

    public function uploadBatch(){
        return view('apps.account.upload_record');
    }

    public function createRecord(){
        return view('apps.account.new_record');
    }

    public function checkIfIdIsValid($data){
        $query = Account::where('deleted_status', '=', '0')
    ...
    public function upload(AccountRequestBatch $request){
    try {
        $records = [];
        $exist = [];
        $noRecord = [];
    ...
    public function update_name(Request $request){
        ...
        try {
            ...
            $record = [
                ...
                $update = Account::where('generated_id', '=', $id)
                                    ->update($record);
                if ($update) {
                $successful = true;
                }else{
...
$query = Account::where('generated_id', '=', $id)
