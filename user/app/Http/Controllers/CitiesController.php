...
class CitiesController extends Controller
{
    ...
    public function manage($id){
        ...
        if ($record['status']=='success') {
            $record = $record['info'];
            return view('apps.cities.manage', compact('record'));
        }else{
            $message = [
                "type" => "info",
                "info" => "",
             ];
            return redirect()->route('list_city')->with('message', json_encode($message));
        }
    }else{
        $message = [
                "type" => "info",
    ...
    public function edit_name($id){
        if (isset($_COOKIE['manageUser']) && ($id === $_COOKIE['manageUser'])) {
            ...
            if ($record['status']=='success') {
                $record = $record['info'];
                return view('apps.cities.edit_name', compact('record'));
            }else{
                $message = [
                "type" => "info",
                "info" => "",
    ...
    public function list(){
        return view('apps.cities.records');
    }

    public function uploadBatch(){
        return view('apps.cities.upload_records');
    }
    ...
    public function list_items()
    {
        ...
        $returnData = '';
        $result = [];
        $query = Cities::where('deleted_status', '=', '0')
                    ->get(['state_name', 'id']);
        if ($query) {
            $returnData = [
                "title" => "Successful",
