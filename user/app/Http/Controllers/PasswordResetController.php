...
class PasswordResetController extends Controller
{
   ...
   {
      return view('apps.auths.forgot_password');
   }
   public function newPasswordLink(Request $request, $token, $user_email)
   {  
      app(\App\Http\Controllers\SessionController::class)->killSession($request);
      $token = base64_decode(base64_decode($token));
      $user_email = base64_decode(base64_decode($user_email));
      $checkToken = $this->checkToken($token, $user_email);
      if ($checkToken['status']) {
         $record = $checkToken['info'];
         setcookie('token', $token, time() + (3600), "/");
         setcookie('user_email', $user_email, time() + (3600), "/");
         return view('apps.auths.change_password', compact('record'))->with("");
      }else{
         $message = [
            'type' => 'error',
            'info' => 'Password reset link has expired, try again.',
         ];
         return redirect()->route('forgotpassword')->with('message', json_encode($message));
      }
   }
   ...
   public function checkToken($token, $userid){
      ...
      $query = VerifyToken::from('verification_tokens as t1')
                        ...
                        ->join('admin_record as t2', 't2.email_one', '=', 't1.user_email')
