...
class RoleController extends Controller
{
    ...
    public function manage($id){
        ...
        if ($record['status']=='success') {
            $record = $record['info'];
            return view('apps.role.manage', compact('record'));
        }else{
            $message = json_encode("No record found");
            return redirect()->route('list_role')->with('message', json_encode($message));
        }
    }

    public function list(){
        return view('apps.role.record');
    }

    public function uploadBatch(){
        return view('apps.role.upload_record');
    }

    public function createRecord(){
        return view('apps.role.new_record');
    }
    ...
    public function create(Request $request){
       $request->validate([
            'role_name' => 'required|unique:roles|max:100|min:2',
        ]);
        try {
            $exist = false;
            $failed = false;
            ...
            $record = [
                ...
                "role_name" => ucfirst(strtolower($request->input('role_name'))),
...
// End loop
    ...
    try {
        ...
        $update = Role::where('generated_id', '=', $id)
                            ->update($row);
...
$query = Role::where('generated_id', '=', $id)
