   ...
   <!-- Please note that, these are just default components, 
    ...
    <div class="container-fluid">
        ...
        <!-- Body component(s) -->
           ...
           <manage_role :server_message="{{ session('message') ?? $message ?? json_encode('') }}" :server_record="{{ session('record') ?? $record ?? json_encode('') }}"></manage_role>
           <edit_role :server_record="{{ session('record') ?? $record ?? json_encode('') }}"> </edit_role>
           <delete_role :server_record="{{ session('record') ?? $record ?? json_encode('') }}"> </delete_role>
    </div>
