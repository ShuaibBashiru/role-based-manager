...
@section('pageTitle', 'Edit Phone number')
@section('content')
   <!-- Please note that, these are just default components, 
    ...
    <div class="container-fluid">
        ...
        <!-- Body component(s) -->
           ...
           <edit_account_phone :server_message="{{ session('message') ?? $message ?? json_encode('') }}" :server_record="{{ session('record') ?? $record ?? json_encode('') }}"></edit_account_phone>
        </div>
