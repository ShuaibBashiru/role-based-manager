...
@section('content')
   <!-- Please note that, these are just default components, 
    ...
    <div class="container-fluid">
        ...
        <manage_salesitems :server_message="{{ session('message') ?? $message ?? json_encode('') }}" :server_record="{{ session('record') ?? $record ?? json_encode('') }}"></manage_salesitems>
        <edit_salesitems :server_record="{{ session('record') ?? $record ?? json_encode('') }}"> </edit_salesitems>
