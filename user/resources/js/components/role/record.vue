...
<div class="border p-2 rounded">
    ...
    <table class="table table-hover table-bordered" id="listTable">
        ...
        <tr>
            ...
            <th scope="col" class="text-truncate" @click="sortOrder('role_name')" title="">Name <i class="bi bi-sort-down float-end"></i></th>
            <th scope="col" class="text-truncate text-center" title="Access">Access</th>
        ...
        <tr v-for="(d, index) in info.slice(startNumber, endNumber)" :key="index">
            ...
            <td class="col text-truncate" v-html="d.role_name ? d.role_name : ''"></td>
            <td class="col text-truncate text-center"> <small><a :href="'../rolepermission/'+d.generated_id">Edit <i class="bi bi-pencil"></i></a></small> </td>
            ...
            <ul class="list-unstyled pb-1 border-bottom" onclick="event.stopPropagation()">
            <li class="mb-2 text-center bg-dark p-1 text-white" v-html="d.role_name"></li>
...
resetForm: function(){
   ...
   validateForm: function(){
    ...
    sendForm: function(){
        ...
        axios.post('/role/create', form).then(response => {
    ...
    manageStatus: function(){
        ...
        $(".toaster").toast('hide')
        this.showOverlay=true;
        axios.get('/role/status/updatebatch', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
             if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
