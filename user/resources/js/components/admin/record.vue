...
<div class="container-fluid">
    ...
    <a href="#" id="topLink"></a>
        ...
        <button type="button" class="btn btn-outline-primary" onclick="location.href='create'">New <i class="bi bi-plus-circle"></i></button>
...
export default {
   ...
   props: ['server_message'],
    ...
    manageStatus: function(){
        ...
        $(".toaster").toast('hide')
        this.showOverlay=true;
        axios.get('/admin/status/updatebatch', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
             if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
