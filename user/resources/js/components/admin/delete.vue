...
<div class="modal-dialog ps-md-3 pe-md-3">
  ...
  <h6 class="alert-heading" v-text="'ID: '+ (info? info.personal_id : '') "></h6>
  <hr>
  <p>
      NAME:  <span class="text-capitalize" v-html="(info? info.lastname + ' ' +info.firstname + ' ' +info.othername : '') "></span> <br/>
      EMAIL: <span class="text-lowercase" v-html="(info? info.email_one : '')"></span>
  </p>
</div>
...
export default {
  ...
  methods:{
    trashItem: function(){
            ...
            this.parameters.email_one = this.info.email_one
            this.parameters.phone_one = this.info.phone_one
            this.parameters.id = this.info.generated_id
            this.button = "Please wait..."
            axios.delete('/admin/trash', {params:this.parameters}).then(response => {
