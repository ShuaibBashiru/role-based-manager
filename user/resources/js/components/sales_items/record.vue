...
<div class="container-fluid">
    ...
    <a href="#" id="topLink"></a>
        ...
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#newSalesItemsModal"> New <i class="bi bi-plus-circle"></i></button>
        <button type="button" class="btn btn-outline-primary" onclick="location.href='upload'">Upload <i class="bi bi-upload"></i></button>
...
<div class="border p-2 rounded mt-1">
    ...
    <table class="table table-hover table-bordered" id="listTable">
        ...
        <tr>
            ...
            <th scope="col" class="text-truncate" @click="sortOrder('item_name')" title="">Item <i class="bi bi-sort-down float-end"></i></th>
            <th scope="col" class="text-truncate" @click="sortOrder('price')" title="">Price <i class="bi bi-sort-down float-end"></i></th>
        ...
        <tr v-for="(d, index) in info.slice(startNumber, endNumber)" :key="index">
            ...
            <td class="col text-truncate" v-html="d.item_name ? d.item_name : ''"></td>
            <td class="col text-truncate" v-html="d.item_price ? d.item_price : ''"></td>
            ...
            <ul class="list-unstyled pb-1 border-bottom" onclick="event.stopPropagation()">
            <li class="mb-2 text-center bg-dark p-1 text-white" v-html="d.item_name"></li>
...
export default {
    name: 'salesItems_List',
   props: ['server_message'],
    data (){
        return{
        pageName: 'Sales Items',
        alertTitle: '',
        alertMsg: '',
        showOverlay: false,
        selectedRecord: [],
...
resetForm: function(){
   ...
   validateForm: function(){
    ...
    sendForm: function(){
        ...
        axios.post('/salesitems/create', form).then(response => {
    ...
    onSearch: function(){
       ...
       for (let i = 0; i < this.totalRecord.length; i++) {
           if (
            this.totalRecord[i].item_name.toString().toLowerCase().search(this.search.toString().toLowerCase())!=-1 ||
    ...
    manageStatus: function(){
        ...
        $(".toaster").toast('hide')
        this.showOverlay=true;
        axios.get('/salesitems/status/updatebatch', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
             if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
