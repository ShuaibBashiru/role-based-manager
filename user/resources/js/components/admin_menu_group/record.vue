...
<div class="border p-2 rounded">
    ...
    <table class="table table-hover table-bordered" id="listTable">
        ...
        <tr>
            ...
            <th scope="col" class="text-truncate" @click="sortOrder('menu_bar')" title="">Menu Bar <i class="bi bi-sort-down float-end"></i></th>
            <th scope="col" class="text-truncate" @click="sortOrder('group_name')" title="">Name <i class="bi bi-sort-down float-end"></i></th>
        ...
        <tr v-for="(d, index) in info.slice(startNumber, endNumber)" :key="index">
            ...
            <td class="col text-truncate" v-html="d.menu_bar ? d.menu_bar : ''"></td>
            <td class="col text-truncate"><i :class="d.group_icon ? d.group_icon : ''"></i> <span v-html="d.group_name ? d.group_name : ''"></span> </td>
...
resetForm: function(){
   ...
   validateForm: function(){
    ...
    sendForm: function(){
        ...
        axios.post('/adminmenugroup/create', form).then(response => {
    ...
    onSearch: function(){
       ...
       for (let i = 0; i < this.totalRecord.length; i++) {
           if (
            this.totalRecord[i].menu_bar.toString().toLowerCase().search(this.search.toString().toLowerCase())!=-1 ||
    ...
    manageStatus: function(){
        ...
        $(".toaster").toast('hide')
        this.showOverlay=true;
        axios.get('/adminmenugroup/status/updatebatch', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
             if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
