...
<div class="container-fluid">
    ...
    <a href="#" id="topLink"></a>
        ...
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#newAccessModal"> New <i class="bi bi-plus-circle"></i></button>
...
resetForm: function(){
   ...
   validateForm: function(){
    ...
    manageStatus: function(){
        ...
        $(".toaster").toast('hide')
        this.showOverlay=true;
        axios.get('/access/status/updatebatch', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
             if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
